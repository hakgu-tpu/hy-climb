...
1. **Data store** (Current, schema migration in progress): Supabase Postgres is the source of truth. Per `docs/wiki/09-decisions.md` `WIKI-DEC-006`, `centers`/`app_config`'s jsonb list/object columns are being replaced by relational tables (`center_prices`, `center_sns_links`, `center_translations`, `events`, `meetings`) and flat columns (`centers.parking_type`/`parking_description`, `app_config.departure_name`/`departure_name_en`/`departure_naver_place_id`). [`src/contexts/DataContext.jsx`](../../src/contexts/DataContext.jsx) reads the new tables (2026-09-19, verified live). `hy-climb-admin` still reads/writes the old jsonb columns, so those columns stay in the schema, unused by this repo, until that repo's cutover lands — see `docs/wiki/07-reference/02-data-schema.md`.
