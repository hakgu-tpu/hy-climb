...
`supabase/migrations/20260919100200_drop_legacy_jsonb_columns.sql` (drops `centers.prices`/`affiliate_prices`/`sns_links`/`parking`/`i18n` and `app_config.departure`/`event`/`meeting`) exists but has deliberately not been run — it's irreversible, and `hy-climb-admin`'s write flows still need manual verification first. See `supabase/README.md` for the exact gate.
